SELECT
    LOWER(TRIM(CAST(product_id AS STRING))) AS product_id,
--    CAST(items.order_item_id AS INT64) AS id,
    COALESCE(LOWER(TRIM(CAST(category.string_field_1 AS STRING))), 'N/A') AS product_category,
    CAST(product_name_lenght AS INT64) AS product_name_length,
    CAST(product_description_lenght AS INT64) AS product_description_length, --corrected spelling
...
QUALIFY ROW_NUMBER() OVER (PARTITION BY product_id ORDER BY product_id DESC) = 1 --dedup